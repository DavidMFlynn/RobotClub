...
// sample code
module BeltPulley(Belt_w = 4,Teeth=42,CenterHole_d=6.35,Dish=false){
	// fits MXL 0.080" pitch belt
	h_teeth=Belt_w*2.5; // scaled(0.4)
	Cable_OD=1; // belt back thickness
	Pulley_h=Belt_w+1+Cable_OD;
	Pitch_d=Teeth*2.5/3.14159; //This is scalled
	Pulley_OD=Pitch_d*0.8+1;
	Flange_w=0.5;

	echo(h_teeth=h_teeth);
	difference(){
		union(){
			// bottom flange
			cylinder(d=Pulley_OD, h=Flange_w+Overlap);
			translate([0,0,Flange_w]) cylinder(d1=Pulley_OD, d2=Pulley_OD-Cable_OD, h=Cable_OD/2+Overlap);
			
			// Belt teeth
			translate([0,0,Flange_w+Cable_OD/2]) scale([0.4,0.4,0.4]) Cog_Pulley(T5, Pitch_d, 370, h_teeth+Overlap, 5); 
			
			// top flange
			translate([0,0,Flange_w+Cable_OD/2+Belt_w])
				cylinder(d2=Pulley_OD, d1=Pulley_OD-Cable_OD, h=Cable_OD/2+Overlap);
			translate([0,0,Flange_w+Cable_OD+Belt_w]) cylinder(d=Pulley_OD, h=Flange_w);
			
			// center hole boss
			if (CenterHole_d!=0)
				translate([0,0,Flange_w+Cable_OD+Belt_w]) cylinder(d=CenterHole_d+6, h=1);
		} // union
		
		if (Dish==true)
			// Dishout
			difference(){
				translate([0,0,2]) cylinder(d=Pulley_OD-6, h=Pulley_h);
				translate([0,0,2-Overlap]) cylinder(d=CenterHole_d+6, h=Pulley_h+Overlap*2);
			} // diff
		
			
		// hole
		if (CenterHole_d!=0)
			translate([0,0,-Overlap]) cylinder(d=CenterHole_d+IDXtra, h=Pulley_h+1+Overlap*2);
	} // diff
...
module PulleyTest(){
	...
	difference(){
		Cog_Pulley(T5, 47.75, 370, h_teeth, 5);
		translate([0,0,-0.05]) cylinder(r=15,h=h_teeth+0.1,$fn=100);
	}
}
//scale([0.4,0.4,0.4])PulleyTest();
...
module FlangedPulley(){
	...
	difference(){
		...
		// setscrew
		translate([-Pitch_r,0,h_teeth+2]) rotate([0,-90,0]) Bolt6Hole(depth=12);
	} // diff
} // FlangedPulley
...
//dog_linear(profile=T5, teeth=5, height=10, t_dog=4);
